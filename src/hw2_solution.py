...
class PIDcontroller(Node):
    def __init__(self, Kp, Ki, Kd):
        ...
        self.subscriber_ = self.create_subscriber(PoseStamped, '/camera_odometry', self.odometry_callback, 10)
    ...
    def odometry_callback(self, robot_odometry):
        x = robot_odometry.pose.position.x
        y = robot_odometry.pose.position.y
        w = robot_odometry.pose.orientation.w

        self.current_state = [x,y,w]
...
# Input: Curr position, desired velocities (global)
# Output: converts desired velocities into a robot centric POV 
