...
class RobotStateEstimator(Node):
    def __init__(self):
        ...
        self.current_state = np.array([MIN_COL, MIN_ROW, 0.0])
        ...
        self.apriltag_world_poses = {
           ...
           'marker_2': (0.0, 0.30, 0.0, -0.5, -0.5, 0.5, 0.5),  # left 
           'marker_7': (0.0, 1.20, 0.0, -0.5, -0.5, 0.5, 0.5),  #left 
           'marker_0': (0.0, 2.10, 0.0, -0.5, -0.5, 0.5, 0.5),        #left  

            'marker_4': (1.80, 2.438, 0.0, -0.707, 0.0, 0.0, 0.707), 
            'marker_1': (0.6048, 2.438, 0.0, -0.707, 0.0, 0.0, 0.707), # (x,y,z, qual_x, qual_y, qual_z, qual_w)

            'marker_3': (2.438, 2.10, 0.0, 0.5, -0.5, 0.5, -0.5), #right
            'marker_5': (2.438, 0.30, 0.0, 0.5, -0.5, 0.5, -0.5), #right
            'marker_6': (2.438, 1.20, 0.0, 0.5, -0.5, 0.5, -0.5),   #right
    ...
    def april_pose_callback(self, msg):
        ...
        # #Find the pose with the least y offset (most accurate pose)
        # min_val = 100000
        # min_idx = -1
        # for idx in range(len(msg.poses)):
        #     pose = msg.poses[idx]
        #     if abs(pose.orientation.y) < min_val:
        #         min_val = abs(pose.orientation.y)
        #         min_idx = idx
        ...
        # tag_id = pose_ids[min_idx]

        # if tag_id not in self.apriltag_world_poses:
        #     return

        
        # pose_camera_apriltag = msg.poses[min_idx]   # syntax: pose_ReferenceFrame_TargetFrame
        ...
        for idx in range(len(msg.poses)):
            ...
            tag_id = pose_ids[idx]
            pose_camera_apriltag = msg.poses[idx]

            if abs(pose_camera_apriltag.orientation.y) > 0.4 or abs(pose_camera_apriltag.position.x) > 0.5:
                continue
            ...
            cstate = np.array([trans_map_camera[0], trans_map_camera[1], angle])

            # if cstate[2] < 0:
            #     cstate[2] = cstate[2] + 6.28
        ...
        if count == 0:
            return
        self.current_state = current_state / count

        # self.queue.append(current_state)# / len(msg.poses)) # enqueue last state
        # if len(self.queue) > 3:
        #     self.queue.pop(0) # pop first elem if full
        # running_sum = [0,0,0]
        # n = 1.0 / (len(self.queue) * (len(self.queue) + 1)) * 2
        # for i in range(len(self.queue)):
        #     running_sum = running_sum + self.queue[i] *n*(i+1)

        # self.current_state = running_sum #/ len(self.queue)
        print(f"CL {used_tags}")
        self.pose_updated = True
...
def main(args=None):
    ...
    pid = PIDcontroller(0.02,0.005,0.06)
    current_state = robot_state_estimator.current_state
    curr_action = "forward"
    target = np.array([MAX_COL, MIN_ROW, 0.0])
    pid.setTarget(target)
    #Current y axis value of sweep 
    row = MIN_ROW
    #Starting x axis of sweep
    col = MIN_COL
    

    while(True):

        if curr_action == "forward":
            pid.kP = 0.027
            pid.kI = 0.0075
            pid.Kd = 0.09

            if (np.linalg.norm(pid.getError(current_state, target)) < 0.1):
                if row == MAX_ROW:
                    curr_action = "done"
                elif col == MIN_COL:
                    col = MAX_COL
                    row += ROW_INC
                    curr_action = "strafe_left"
                    target = np.array([col, row, 0.0])
                    pid.setTarget(target)

                elif col == MAX_COL:
                    col = MIN_COL
                    row += ROW_INC
                    curr_action = "strafe_right"
                    target = np.array([col, row, np.pi])
                    pid.setTarget(target)

        elif curr_action == "strafe_right":
            ...
            if (np.linalg.norm(pid.getError(current_state, target)) < 0.1):
                curr_action = "turn_around"
                target = np.array([col, row, -0.15])
                pid.setTarget(target)

        elif curr_action == "strafe_left":
            ...
            if (np.linalg.norm(pid.getError(current_state, target)) < 0.1):
                curr_action = "turn_around"
                target = np.array([col, row, np.pi-0.15])
                pid.setTarget(target)


       
        elif curr_action ==  "turn_around":

            pid.kP = 0.045
            pid.kI = 0.007
            pid.Kd = 0.09
  
            if (np.linalg.norm(pid.getError(current_state, target)) < 0.05):
                
                if col == MIN_COL:
                    curr_action = "forward"
                    target = np.array([MAX_COL, row, 0.0])
                    pid.setTarget(target)

                elif col == MAX_COL:
                    curr_action = "forward"
                    target = np.array([MIN_COL, row, np.pi])
                    pid.setTarget(target)

        
        elif curr_action == "done":
            break
        ...
        if found_state and curr_action != "turn_around": # if the tag is detected, we can use it to update current state.
            current_state = estimated_state
        print(f"{current_state} - {curr_action}  - {target}")
    ...
    # # init pid controller
    # # old: (0.015,0.005,0.07)
    # pid = PIDcontroller(0.023,0.007,0.09)
    # current_state = robot_state_estimator.current_state
