...
class RobotStateEstimator(Node):
    def __init__(self):
        ...
        self.current_state = np.array([1.982,0.4572,np.pi/2])
        ...
        self.apriltag_world_poses = {
            ...
            'marker_2': (0.0, 0.6048, 0.0, -0.5, -0.5, 0.5, 0.5),
            'marker_3': (2.438, 1.80, 0.0, 0.5, -0.5, 0.5, -0.5),
            'marker_4': (1.80, 2.438, 0.0, -0.707, 0.0, 0.0, 0.707), 
            'marker_5': (2.438, 0.6048, 0.0, 0.5, -0.5, 0.5, -0.5),
            'marker_6': (1.80, 0.0, 0.0, 0.0, -0.707, 0.707, 0.0),
            'marker_7': (0.6048, 0.0, 0.0, 0.0, -0.707, 0.707, 0.0),
            'marker_0': (0.0, 1.80, 0.0, -0.5, -0.5, 0.5, 0.5),
    ...
    def april_pose_callback(self, msg):
        ...
        for idx in range(len(msg.poses)):
            ...
            pose_camera_apriltag = msg.poses[min_idx]

            if abs(pose_camera_apriltag.orientation.y) > 0.15 or abs(pose_camera_apriltag.position.x) > 0.5:
                continue
            count += 1
        ...
        if count == 0:
            return
        current_state = current_state / count
...
def main(args=None):
    ...
    # waypoint = np.array([
    #     # [1.982,0.4572,np.pi/2], 
    #     [0.606,0.4572,np.pi],
    #     [0.422, 0.762, np.pi/2],
    #     [0.4572, 1.982, np.pi/2]
    ...
    waypoint = np.array([
        ...
        [1.9,0.45,np.pi], 
        [1.2,0.45,np.pi],
        [1.2,0.45,6.0 * np.pi/8.0],
        [0.7,0.95,6.0 * np.pi/8.0],
        [0.6,0.95, np.pi/2],
        [0.6,1.9, np.pi/2]
  
        # [1.2,0.4572,3 * np.pi/4], 
        # [0.8, 0.8, 3 * np.pi/4],
        # [0.8, 1.5, np.pi/2],
        # [0.4572, 1.982, np.pi/2]
